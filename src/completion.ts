...
function trace(param: unknown) {
  if (gDebug) {
    ...
    const stream = fs.createWriteStream(
      util.getStringOrThrow(process.env.FAB_COMPLETION_LOG),
      { flags: "a+" },
    );
...
): commander.CommandUnknownOpts | undefined {
...
function processEnv(): CompletionContext {
  ...
  const compPoint = Number(
    util.getStringOrThrow(process.env.COMP_POINT, "COMP_POINT"),
  );
  ...
  const lookingForOption =
    partial.startsWith("-") && lineToPoint.indexOf(" -- ") === -1;
...
function complete(program: commander.Command) {
  ...
  if (context.commandName === undefined) {
    const programEventEmitter = program as commander.Command &
      events.EventEmitter;
    ...
    const command = findCommand(
      context.commandName,
      program,
    ) as commander.Command & events.EventEmitter;
...
export function completion(program: commander.Command): void {
  ...
  if (
    ...
    const completionScript = fs.readFileSync(
      path.join(__dirname, "../../resources/fab_completion.sh"),
    );
